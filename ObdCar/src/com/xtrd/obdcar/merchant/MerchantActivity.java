...
public class MerchantActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initTitle(0, R.drawable.btn_back_bg,R.string.title_merchants, 0,0);
		initView();
		startRefresh();
	}
	...
	private void initView() {
		...
		View view = LayoutInflater.from(this).inflate(R.layout.layout_merchant_top, null);
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
					if (view.getLastVisiblePosition() == view.getCount() - 1) {
						if (hasMore) {
							getMerchants(longitude, latitude, currentCity);
						}
					}
				}
			}
	...
	private void getLocation() {
		...
		if(SettingLoader.getCarLatLng(this)!=null) {
			getMerchants(SettingLoader.getCarLatLng(this).longitude, SettingLoader.getCarLatLng(this).latitude, SettingLoader.getCurrentCity(this));
		}else {
			XtrdApp.getInstance().getCurrentLocation(
					new XtrdApp.LocationCallBack() {
						...
						public void callback(double lon, double lat, String city) {
							...
							latitude = lat;
							currentCity = city;
							getMerchants(longitude, latitude, city);
						}
					});
		}
	...
	private void getMerchants(double longitude, double latitude, String city) {
		...
		fh.post(ApiConfig.getRequestUrl(ApiConfig.Merchants_Url), params,new AjaxCallBack<String>() {
					...
					if (!StringUtils.isNullOrEmpty(str)) {
						...
						if (json.has("status")) {
							...
							if (1 == status) {
								...
								Utils.showToast(MerchantActivity.this,getResources().getString(R.string.tips_no_merchants));
							}
							updateUI();
						}
					}

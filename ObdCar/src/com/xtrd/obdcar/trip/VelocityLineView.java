...
public class VelocityLineView {
	...
	public static GraphicalView getLineView(Context context,ArrayList<FuelReport> list) {
		...
		for(int i=0;i<list.size();i++) {
			report = list.get(i);
			yValues[i] = report.getVelocity();
			if(maxVelocity<report.getVelocity()) {
				maxVelocity = report.getVelocity();
			}
		}
		...
		setChartSettings(renderer, "速度时间曲线", "时间", "速度(公里/小时)", -1, list.size(), -1, maxVelocity+2, Color.BLACK, Color.BLACK);

...
public class ChooseDrivingActivity extends BaseActivity {
	...
	private ObdLocation mLocation;

	public ChooseDrivingActivity() {
		layout_id = R.layout.activity_driving;
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(from) {
			initTitle(0, R.drawable.btn_back_bg,
					R.string.text_car_driviing, R.string.btn_finish,0);
		}else {
			initTitle(0, R.drawable.btn_back_bg,
					R.string.title_address,R.string.btn_finish,0);	
		}
	...
	protected void showChooseDialog(final ObdLocation location) {
		...
		.setTitle("提示信息")
		.setMessage("确定选择   " + location.getDisplayName()+(mFrag.getChooseCity()!=null?" "+mFrag.getChooseCity().getDisplayName():""))
		...
		.setNegativeButton("确定", new ObdDialog.OnClickListener() {
			...
			@Override
			public void onClick(DialogInterface dialog, int which) {
				choose_text.setText(location.getDisplayName()+(mFrag.getChooseCity()!=null?" "+mFrag.getChooseCity().getDisplayName():""));
				Intent intent = new Intent();
				intent.putExtra("areaCode", (mFrag.getChooseCity()!=null?mFrag.getChooseCity().getAreaCode():location.getAreaCode()));
	...
	private void getDrivingArea() {
		...
		fh.get(ApiConfig.getRequestUrl(ApiConfig.City_Url), new AjaxCallBack<String>(){
			...
			public void onFailure(Throwable t, int errorNo, String strMsg) {
				dismissLoading();
				Utils.showToast(ChooseDrivingActivity.this, getResources().getString(R.string.data_load_fail));
	...
	public void onClick(View v) {
		...
		case R.id.btn_right:
			if(mLocation==null) {
				Utils.showToast(this, getResources().getString(R.string.tips_choose_province));
			}else {
				showChooseDialog(mLocation);
			}
			break;
		default:
			break;

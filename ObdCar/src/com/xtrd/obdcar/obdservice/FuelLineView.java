...
public class FuelLineView {
	...
	public static GraphicalView getBarView(Context context,ArrayList<FuelReport> list,ArrayList<String> xLabels,int currentTabIndex, boolean sameyear) {
		...
		double maxVelocity = 0,fuel100=0;
		for(int i=0;i<list.size();i++) {
			if(i<list.size()) {
				report = list.get(i);
			}
			fuel100 = (double)(report.getFuelAmount()*100/report.getDistance());
			yValues[getPosition(report, xLabels, currentTabIndex,sameyear)] = fuel100;
			if(maxVelocity<fuel100) {
				maxVelocity = fuel100;
			}
		}
		...
		setChartSettings(renderer, "", "时间", "油耗(升/百公里)", 0, xLabels.size()+1, 0, maxVelocity+1, Color.BLACK, Color.BLACK);
		...
		renderer.setPanLimits(new double[] { 0, xLabels.size()+1, 0, maxVelocity});
		renderer.setZoomLimits(new double[] { 0, xLabels.size(), 0, maxVelocity+1});

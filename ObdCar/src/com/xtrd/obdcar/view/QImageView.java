...
public class QImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > Utils.getScreenWidth(context)) {
			// 如果图宽大于屏宽，就不用水平居中
			center(false, true);
		} else if(height > Utils.getScreenHeight(context)){
			center(true, false);
		}else{
			center(true, true);
		}
	...
	private void arithScaleRate() {
		float scaleWidth = Utils.getScreenWidth(context) / (float) imageWidth;
		float scaleHeight = Utils.getScreenHeight(context) / (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		//缩放到屏幕大小
		zoomTo(scaleRate,Utils.getScreenWidth(context) / 2f, Utils.getScreenHeight(context) / 2f);
	...
	public void layoutToCenter()
	{
		...
		//空白区域宽高
		float fill_width = Utils.getScreenWidth(context) - width;
		float fill_height = Utils.getScreenHeight(context) - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom =( Utils.getScreenWidth(context)/2f)/imageWidth;

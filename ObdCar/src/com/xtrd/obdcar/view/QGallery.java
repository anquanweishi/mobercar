...
public class QGallery extends Gallery {
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
		...
		if (view instanceof QImageView) {
			...
			if ((int) width <= Utils.getScreenWidth(context) && (int) height <= Utils.getScreenHeight(context))// 如果图片当前大小<屏幕大小，直接处理滑屏事件
			{
				if(isScroll)
				...
				{
					if (right <= Utils.getScreenWidth(context)) {
						super.onScroll(e1, e2, distanceX, distanceY);
					}else {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (view instanceof QImageView) {
				...
				if ((int) width <= Utils.getScreenWidth(context) && (int) height <= Utils.getScreenHeight(context))// 如果图片当前大小<屏幕大小，判断边界
				{
					break;
				}
				float v[] = new float[9];
				...
				if (bottom < Utils.getScreenHeight(context)) {
					imageView.postTranslateDur(Utils.getScreenHeight(context) - bottom, 200f);
				}
			}
			break;
		}
		...
		public boolean onDoubleTap(MotionEvent e) {
			...
			if (view instanceof QImageView) {
				...
				if (imageView.getScale() > imageView.getScaleRate()) {
					imageView.zoomTo(imageView.getScaleRate(), Utils.getScreenWidth(context) / 2, Utils.getScreenHeight(context) / 2, 200f);
					// imageView.layoutToCenter();
				} else {
					imageView.zoomTo(1.0f, Utils.getScreenWidth(context) / 2, Utils.getScreenHeight(context) / 2, 200f);
				}
			} else {
			}
			return true;
